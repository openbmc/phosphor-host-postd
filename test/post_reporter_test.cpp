...
{
    PostReporter testReporter(bus, SNOOP_OBJECTPATH, true);
    std::vector<uint64_t> secondaryCode = {123, 124, 125};
...
{
    PostReporter testReporter(bus, SNOOP_OBJECTPATH, true);
    std::vector<uint64_t> secondaryCode = {10, 400, 0, 345, 5678};
    ...
    testReporter.value(std::make_tuple(456, secondaryCode));
    EXPECT_EQ(456, std::get<0>(testReporter.value()));
    EXPECT_EQ(secondaryCode,
              std::get<SECONDARY_POST_CODE>(testReporter.value()));

    secondaryCode = {23000, 200, 0, 345, 2};
    testReporter.value(std::make_tuple(0, secondaryCode));
    EXPECT_EQ(0, std::get<0>(testReporter.value()));
    EXPECT_EQ(secondaryCode,
              std::get<SECONDARY_POST_CODE>(testReporter.value()));

    secondaryCode = {10, 400, 0, 345, 5678};
    testReporter.value(std::make_tuple(456, secondaryCode));
    EXPECT_EQ(456, std::get<0>(testReporter.value()));
    EXPECT_EQ(secondaryCode,
              std::get<SECONDARY_POST_CODE>(testReporter.value()));
