...
{
    public:
        PostReporter(sdbusplus::bus::bus& bus,
                     const char* objPath,
                     bool defer)
...
/*
 * 256 bytes is a nice amount.  It's improbable we'd need this many, but its gives us leg room in
 * the event the driver poll doesn't return in a timely fashion.  So, mostly arbitrarily chosen.
 */
...
int main(int argc, char* argv[])
{
    ...
    auto reporter = std::make_unique<PostReporter>(
        bus, snoopObject, deferSignals);
     ...
     *
     * TODO(venture): There may be a way to use sdevent to poll both the file and the dbus in the
     * same event loop.  If I could get the sdbus pointer from bus directly, I'd grab a file
     * handler from it, and then just poll on both in one loop.  From a cursory look at sdevent, I
     * should be able to do something similar with that at some point.
     */
